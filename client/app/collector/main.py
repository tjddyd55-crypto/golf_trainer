...
def init_module_if_needed():
    ...
    # 매장별 좌표 파일 로드 (GUI에서 전달받는 경우 파일 없어도 됨)
    ...
    try:
        ...
        # GUI에서 좌표를 전달받는 경우 파일이 없어도 됨 (run() 함수에서 regions 파라미터로 받음)
        early_log(f"regions file load failed: {e} (GUI에서 전달받는 경우 무시 가능)")
        REGIONS = {}  # 빈 딕셔너리로 초기화 (run()에서 regions 파라미터로 덮어씀)
            ...
            try:
                ...
                if state == "WAITING" and pending_read_at is not None:
                    ...
                    if now >= pending_read_at:
                        ...
                        active_user = get_active_user(current_store_id, current_bay_id)
                        if not active_user:
                            # active_user가 없으면 경고 로그 출력 및 샷 저장 보류
